...
public class ArvoreAVL<G extends Comparable<G>> {
	...
	private int calcularAltura(NoAVL<G> no) {
		...
		/**
		 * Verifico se nó direito existe, então obtenho a altura.
		 */
		if(no.getDireito() != null){
			alturaDir = no.getDireito().getAltura();
		}

		/**
		 * Verifico se nó esquerdo existe, então obtenho a altura..
		 */
		if (no.getEsquerdo() != null){
			alturaEsq = no.getEsquerdo().getAltura();
		}
		/**
	...
	private NoAVL<G> rotacaoDireita(NoAVL<G> alvo) {
		 ...
		 */
		alvo.setAltura(calcularAltura(alvo));
		substituto.setAltura(calcularAltura(substituto));
